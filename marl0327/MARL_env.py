...
class TODCMARLEnv(gym.Env):
    ...
    def step(self, action_dict):
        """RL 对外一步。decision 模式内层按 main0319：update -> _phase_geometry_step -> _phase_check_decision 循环直至重规划或终止。"""
        ...
        if self._episode_terminal_pending:
            self._episode_terminal_pending = False
            rewards, reward_details = self._compute_rewards(action_indices, sim_time_elapsed=0.0)
    ...
    def _advance_from_paths(self):
        ...
        for id,eid in enumerate(self.UnCapEidNew):
    ...
    def _apply_hungarian_and_paths(self):
        ...
        self.pairs_realE2P=np.array([[self.UnCapEid[pairs_realE2P_[i,0].astype(int)],self.UnCapPid[pairs_realE2P_[i,1].astype(int)] ]
    ...
    # --- main0319 内层单步分解：update -> _phase_geometry_step -> check（Gym 的 step(action) 仍是对外 RL 接口）---

    def _phase_update(self) -> None:
        """与 main0319 第 204-216 行一致：时间推进并同步未捕获配对。"""
    ...
    def _phase_check_decision(self) -> Tuple[bool, bool]:
        ...
        """内层检查：返回 (need_replan, terminal)，语义同 _phase_check_decision。"""
    ...
    def _update_capflag_from_geometry(self) -> int:
        ...
        distances_now = self._pairwise_dist()
                # --- Step 2. 角度差与代价 (向量化广播) ---
        # dx, dy 均为 (nE, nP) 矩阵
        dx = self.PosE[:, 0] - self.PosP[:, 0]
        dy = self.PosE[:, 1] - self.PosP[:, 1]
        ...
        # 利用广播: (nE, nP) - (nP,) -> (nE, nP)
        angleDiff_ = angPE - np.degrees(self.PosP[:,2])
    ...
    def _single_inner_simulation_tick(self, dummy_actions: Optional[np.ndarray] = None) -> Tuple[bool, bool]:
        ...
        Returns (need_replan, episode_terminal).
        """
        max_t = float(self.length_E_max) / float(self.v_E)
        if self.t >= max_t - self._t_all_at_path_start - 1e-9:
            return False, True
