...
while not np.all(distances < CapDist):
    ...
    UnCapEid = np.sort(pairs_realE2P[~Capflag, 0].astype(int))  # 保持未捕获 Evader 的 ID 顺序
...
############################################################从这转化
    ...
    P_pos_list = [
        path[:, min(curr_idx_p, path.shape[1]-1)] 
        for path in PathP
    ]
...
#########################################################
    ...
    if not flagIn:
        ...
        pairsE2Val=[]
        for i, eid in enumerate(UnCapEid):
            pairsE2ValT_[eid]=pairsE2Val_[i, :]
        
        for i, eid in enumerate(UnCapEid):
            pairsE2Val.append(pairsE2Val_[eid, :])
...
##############################################从这里开始转化
            ...
            # 提取 [Eid, Pid] 配对
            pairs_realE2P = AssignedIntercepts[:, [11, 12]]
...
# --- 1. 清理与基础地图绘制 ---
    ...
    for idx, eid in enumerate(UnCapEid):
        ...
        # --- 绘制预测轨迹 (虚线表示预测) ---
        ax.plot(PathP[pid][0, :], PathP[pid][1, :], '-', color=color_p, linewidth=2, alpha=1)
        ax.plot(PathE[eid][0, :], PathE[eid][1, :], '--', color=color_e, linewidth=2, alpha=1)
        ...
        ax.plot(BestPaths[eid][0, :], BestPaths[eid][1, :], '-', color=color_p, linewidth=2.5, zorder=3)
