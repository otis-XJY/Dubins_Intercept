...
def main():
    ...
    # 每个 evader 对应的设施点 id（1-based，与旧脚本一致）；长度需等于 num_E
    ...
    for tm in time_maps:
        ...
        ValuePosOb = ValuePos[value_id - 1, :]
        S = Evader[:, 0:2]
        E = ValuePosOb[:, 0:2]
        ...
        for _k in range(m_profiles):
            ...
            for i in range(S.shape[0]):
                ...
                path_xyz = _interpolate_path(pts, stepsize=Stepsize, theta0=float(Evader[i, 2]))
            ...
            # 计算从 Evader 到 ValuePos 的所有路径 (flagAll=1)
            ...
            print(f"[EPath] ok TimeMap={tm} TimeEpath={time_epath} dir={save_dir}")
